...
template <std::size_t Degree> struct SpreadSubproblemPoly2DW8 {
    ...
    void process_4(
        ...
        compute_kernel(_mm512_permute_ps(zd, 0b11111111), strengths + 6, vx, vy);
...
template <std::size_t Degree> struct SpreadSubproblemPoly2DW8F64 {
    ...
    SpreadSubproblemPoly2DW8F64(U const *coefficients, std::size_t width)
        : coefficients(allocate_aligned_array<double>((Degree + 1) * 8, 64)),
          kernel_width(static_cast<double>(width)),
          writeout_width(width) {
     ...
     * and pre-multiplies the x-axis values by the complex weights.
     *
     * @param vx1[out] The lower half of the x-axis kernel, multiplied by the complex strength, in interleaved format.
     * @param vx2[out] The upper half of the x-axis kernel, multiplied by the complex strength, in interleaved format.
     * @param vy[out] The y-axis kernel without multiplication.
     *
     */
    void compute_kernel(__m512d z, double const *strengths, __m512d &vx1, __m512d &vx2, __m512d &vy) const {
        ...
        vx1 = _mm512_permutex2var_pd(kx_re, _mm512_setr_epi64(
            re | 0, im | 0, re | 1, im | 1, re | 2, im | 2, re | 3, im | 3), kx_im);
        vx2 = _mm512_permutex2var_pd(kx_re, _mm512_setr_epi64(
            re | 4, im | 4, re | 5, im | 5, re | 6, im | 6, re | 7, im | 7), kx_im);

        vy = ky;
    }

    void accumulate_strengths(
        double *output, int ix, int iy, std::size_t ldx, __m512d vx1, __m512d vx2, __m512d vy) const {
        // Compute index as base index to aligned location
        // and offset from aligned location to actual index.
        int i_aligned = ix & ~3;
        int i_remainder = ix - i_aligned;
    ...
    void process_4(
        ...
        __m512d z1 = _mm512_permutexvar_pd(
            _mm512_setr_epi64(0, 4, 1, 5, 0, 4, 1, 5),
            z);
        __m512d z2 = _mm512_permutexvar_pd(
            _mm512_setr_epi64(2, 6, 3, 7, 2, 6, 3, 7),
            z);
        ...
        _mm512_store_epi64(
            indices,
            _mm512_sub_epi64(
                xy_ceili, _mm512_inserti64x4(_mm512_set1_epi64(offset_x), _mm256_set1_epi64x(offset_y), 1)));
