...
void gather_rescale(benchmark::State& state) {
    ...
    auto const &points_arg =
        points.cast([](auto &&x) { return static_cast<T const *>(x.get()); });

    for (auto _ : state) {
        finufft::spreading::gather_and_fold<Dim, int64_t, T>(
            output,
            points_arg,
            sizes,
            permutation.data(),
...
BENCHMARK(gather_rescale<1, double>)->RangeMultiplier(16)->Range(2 << 12, 2 << 18);
BENCHMARK(gather_rescale<2, double>)->RangeMultiplier(16)->Range(2 << 12, 2 << 18);
BENCHMARK(gather_rescale<3, double>)->RangeMultiplier(16)->Range(2 << 12, 2 << 18);

BENCHMARK(gather_rescale<1, float>)->RangeMultiplier(16)->Range(2 << 12, 2 << 18);
BENCHMARK(gather_rescale<2, float>)->RangeMultiplier(16)->Range(2 << 12, 2 << 18);
BENCHMARK(gather_rescale<3, float>)->RangeMultiplier(16)->Range(2 << 12, 2 << 18);
