...
struct SpreadSubproblemPoly3DW8 : PolynomialBatchHolder<float, 16, Degree> {
    ...
    void compute_kernel(
        ...
        __m512 k_z = horner_polynomial_evaluation<Degree>(z, this->coefficients.get());
    ...
    void accumulate_strengths(
        ...
        for (std::size_t dz = 0; dz < this->width; ++dz) {
            for (std::size_t dy = 0; dy < this->width; ++dy) {
    ...
    void process_4(
        ...
        std::size_t stride_z, std::integral_constant<bool, Partial>, std::uint32_t mask) const {
        ...
        if (Partial) {
            ...
            z = _mm_maskz_load_ps(mask, coord_z);
        } else {
            x = _mm_load_ps(coord_x);
            y = _mm_load_ps(coord_y);
            z = _mm_load_ps(coord_z);
        }
    ...
    void operator()(
        ...
        auto initial_elements_missing =
            align_multiple_pointers_previous(16, coord_x, coord_y, coord_z);
    ...
    std::array<KernelWriteSpec<float>, 3> target_padding() const {
        ...
        return {
            ...
            {ns2, 0, static_cast<int>(this->width)},
            {ns2, 0, static_cast<int>(this->width)}};
    }
};
...
template <std::size_t Degree>
struct SpreadSubproblemPoly3DW8F64 : PolynomialBatchHolder<double, 8, Degree> {
    ...
    void compute_kernel(
        ...
        __m512d k_y = horner_polynomial_evaluation<Degree>(y, this->coefficients.get());
        _mm512_store_pd(vy, k_y);

        __m512d k_z = horner_polynomial_evaluation<Degree>(z, this->coefficients.get());
        _mm512_store_pd(vz, k_z);

        __m512d k_x = horner_polynomial_evaluation<Degree>(x, this->coefficients.get());
    ...
    void process_4(
        ...
        std::size_t stride_y, std::size_t stride_z, std::integral_constant<bool, Partial>,
        uint32_t mask) const {
        ...
        if (Partial) {
            ...
            z = _mm512_castpd256_pd512(_mm256_maskz_load_pd(mask, coord_z));
        } else {
            x = _mm256_load_pd(coord_x);
            y = _mm256_load_pd(coord_y);
            z = _mm512_castpd256_pd512(_mm256_load_pd(coord_z));
        }
    ...
    void operator()(
        ...
        auto initial_elements_missing =
            align_multiple_pointers_previous(32, coord_x, coord_y, coord_z);
        strengths -= 2 * initial_elements_missing;
