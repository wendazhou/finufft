...
void benchmark_spread_subproblem_1d_impl(benchmark::State& state, Fn&& subproblem_impl) {
    ...
    for(auto _ : state) {
        ...
        subproblem_impl(0, num_points, result.data(), positions.size(), positions.data(), strengths.data(), opts);
        benchmark::ClobberMemory();
...
void benchmark_spread_subproblem_1d(benchmark::State& state) {
    benchmark_spread_subproblem_1d_impl(state, spread_subproblem_1d);
}

void benchmark_spread_subproblem_1d_avx2(benchmark::State& state) {
    benchmark_spread_subproblem_1d_impl(state, finufft::spread_subproblem_1d_avx2_7<float>);
}
...
BENCHMARK(benchmark_spread_subproblem_1d_avx2)->RangeMultiplier(4)->Range(128, 1<<14)->Unit(benchmark::kMicrosecond);
