...
def write_config_source(configs: Iterable[Tuple[int, float, int]], fp=sys.stdout):
    deg_width = set((degree, width) for degree, beta, width in configs)
    deg_width = sorted(deg_width)

    print("// Data generated by make_kernel_weights.py - do not directly modify!\n", file=fp)
    print("// This file is used by precomputed_poly_kernel_data.h - do not directly include!\n", file=fp)
    print("typedef std::tuple<", file=fp)
    print(",\n".join(f"poly_kernel_config<{degree},{width}>" for (degree, width) in deg_width), file=fp)
    print("> poly_kernel_configs_t;", file=fp)
...
def main():
    ...
    with open(os.path.join(root, "../src/precomputed_poly_kernel_configs.inl"), "w") as fp:
        write_config_source(configs, fp)
