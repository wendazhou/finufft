...
#define INVOKE_GATHER_AND_FOLD_FN(Fn, Dim, T)                                                      \
    [](finufft::spreading::SpreaderMemoryInput<Dim, T> const &memory,                              \
       ...
       std::int64_t const *sort_indices,                                                           \
...
TYPED_TEST_P(GatherRescaleFixture, GatherRescaleIdentity) {
    ...
    auto result = run_spreader<Dim>(num_gather_points, points_arg, permutation.data(), FnType{});
...
REGISTER_TYPED_TEST_SUITE_P(GatherRescaleFixture, GatherRescaleIdentity);
INSTANTIATE_TYPED_TEST_SUITE_P(GatherRescaleIdentity, GatherRescaleFixture, GatherRescaleTestsParameters);

#undef INVOKE_GATHER_AND_FOLD_FN
